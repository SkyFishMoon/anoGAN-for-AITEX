...
if __name__ == "__main__":
	...
	parser.add_argument('--anomaly', dest='anomaly', help="switch for anomaly detecting", action='store_true', default=False)
	parser.add_argument('--train_anomaly', dest='train_anomaly', help="switch for anomaly detecting during training and output accuracy score", action='store_true',
						default=True)
	...
	if ',' in config.image_size:
		config.image_size = [int(s.strip()) for s in config.image_size.split(',')]
		...
		for epoch in range(config.epoches):
			...
			if args.train_anomaly:
				...
				for i, (test_input, label, path) in enumerate(test_loader):
					...
					optimizerZ = torch.optim.Adam([ano_z], lr=config.base_lr,
												  betas=(config.beta1, 0.999))  # [ano_z] : to make list for parameter
					z_loss_list = []
					...
					for epoch in range(config.test_epoches):
						...
						z_loss_list.append(loss.item())
						optimizerZ.zero_grad()
						loss.backward()
						optimizerZ.step()
						if epoch in save_epoch:
							print("{} epoch anomaly score of {}: {}".format(epoch + 1, path[0].split('/')[-1],
																			z_loss_list[-1]))
					loss_list.append(min(z_loss_list))
				min_loss = min(loss_list)
				max_loss = max(loss_list)
				max_f1 = 0.0
				best_threshold = 0.0
				for i in range(round(min_loss), round(max_loss), 100):
					predict = np.where(np.array(loss_list) > i, 0, 1).tolist()
					f1score = f1_score(label_list, predict, average='binary')
					if f1score > max_f1:
						max_f1 = f1score
						best_threshold = i
				predict = np.where(np.array(loss_list) > best_threshold, 0, 1).tolist()
				acc_score = accuracy_score(label_list, predict)
				acc_score_list.append(acc_score)
				plot_loss(epoch + 1, config.epoches, args.save_dir, acc_score=acc_score_list)
	...
	if args.anomaly:
		...
		for i, (test_input, label, path) in enumerate(test_loader):
			...
			loss_list.append(min(z_loss_list))
		fpr, tpr, thresholds = roc_curve(label_list, loss_list)
		plt.plot(fpr, tpr, lw=1.5, label="W-Y, AUC=%.3f)" % auc(fpr, tpr))
		plt.xlabel("FPR", fontsize=15)
		plt.ylabel("TPR", fontsize=15)

		plt.title("ROC")
		plt.legend(loc="lower right")
		plt.savefig(args.test_result_dir + '/' + 'roc_curve.jpeg')
		min_loss = min(loss_list)
		max_loss = max(loss_list)
		max_f1 = 0.0
		best_threshold = 0.0
		for i in range(round(min_loss), round(max_loss), 100):
			...
			if f1score > max_f1:
				...
				best_threshold = i
		predict = np.where(np.array(loss_list) > best_threshold, 0, 1).tolist()
		p = precision_score(label_list, predict, average='binary')
		r = recall_score(label_list, predict, average='binary')
		f1score = f1_score(label_list, predict, average='binary')

		confusion_mat = confusion_matrix(label_list, predict)
		...
		plt.savefig(args.test_result_dir + '/' + 'confusion_matrix.jpeg')
		# need filename!
