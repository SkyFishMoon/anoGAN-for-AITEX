    ...
    for root, _, fnames in sorted(os.walk(dir)):
        for fname in sorted(fnames):
            if is_image_file(fname):
                ...
                if 'test' in path and 'defect' in path:
                    item = (path, 0)
                else:
                    item = (path, 1)
                images.append(item)
...
def plot_loss(num_epoch, epoches, save_dir, acc=False, **loss):
    fig, ax = plt.subplots() 
    ax.set_xlim(0, epoches + 1)
    if len(loss) == 2:
        ax.set_ylim(0, max(np.max(loss['g_loss']), np.max(loss['d_loss'])) * 1.1)
    elif len(loss) == 1 and not acc:
        ax.set_ylim(0, max(np.max(loss['z_loss'])) * 1.1)
    elif len(loss) == 1 and acc:
        ax.set_ylim(0, max(np.max(loss['acc_score'])) * 1.1)
    ...
    if len(loss) == 2:
        ...
        plt.plot([i for i in range(1, num_epoch + 1)], loss['acc_score'], label='acc_score', color='red', linewidht=3)
        plt.legend()
        plt.savefig(os.path.join(save_dir, 'DCGAN_acc_epoch_{}.png'.format(num_epoch)))
...
def create_gif(epoches, save_dir, name):
    if name == "dcgan":
        ...
        images = []
        for i in range(1, epoches + 1):
            images.append(imageio.imread(os.path.join(save_dir, 'DCGAN_acc_epoch_{}.png'.format(i))))
        imageio.mimsave(os.path.join(save_dir, 'DCGAN_result_acc.gif'), images, fps=5)
